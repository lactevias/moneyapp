...
import { calculateMultiCurrencyTotal, formatCurrency } from "@/lib/currency";
...
  savings?: number; // Changed to optional
  transactions: Transaction[]; // Keep this required as App.tsx provides it
  goalsCount?: number;
...
  const categoryTotals = (transactions || []) // Added guard for safety
...
          subtitle={goalsCount ? `${goalsCount} active goals` : undefined}
...
              s          {formatCurrency(amount, 'RUB')} ({percentage.toFixed(1)}%)
                        </span>
                      </div>
